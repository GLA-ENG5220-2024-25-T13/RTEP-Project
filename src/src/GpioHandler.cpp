...
void GpioHandler::stopMonitoring() {
    if (running.exchange(false)) { // Atomically set running to false and check previous value
...
void GpioHandler::monitorLoop() {
    ...
    while (running.load()) {
        ...
        if (ret < 0) { // Error
            ...
            if (alarmController.isArmed()) { // Quick check before locking
